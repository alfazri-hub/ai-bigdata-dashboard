...
import {
  DollarSign, AlertTriangle, CheckCircle, Clock, Server, Globe, Calendar, ToggleLeft, Cpu, Activity, Zap, CreditCard, HardDrive, Layers
} from "lucide-react";
  ...
  if (!predictionResult) {
    return (
      <div className="p-6 sm:p-10 max-w-[800px] mx-auto w-full text-center py-24 space-y-6 animate-fade-in">
        <div className="w-16 h-16 bg-blue-50 rounded-full flex items-center justify-center mx-auto text-blue-500 border border-blue-100 shadow-inner">
          <DollarSign size={28} className="animate-bounce" />
        </div>
        <div className="space-y-1.5">
          <h2 className="text-xl font-black text-slate-800 tracking-tight">Belum Ada Hasil Estimasi</h2>
        ...
        <div className="pt-2">
          <Button onClick={() => router.push("/input")} className="px-6 py-2.5 rounded-xl bg-gradient-to-r from-blue-500 to-cyan-500 text-white font-black text-xs uppercase tracking-wider cursor-pointer">
            Kalkulasi Baru
          </Button>
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-slate-200/40 pb-4">
        <div className="min-w-0 flex-1">
          ...
          <p className="text-slate-500 text-xs mt-1 truncate font-semibold flex items-center gap-1.5">
            ...
            Waktu Prediksi: <span className="text-blue-600 font-extrabold">{predictionResult.timestamp}</span>
            <span className="text-slate-300 mx-1">•</span>
            <span className="text-slate-600 font-extrabold">{input.Region}</span>
          </p>
        </div>
        <Button onClick={() => router.push("/input")} className="rounded-xl text-xs font-black cursor-pointer shrink-0">
          Kalkulasi Baru
        </Button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-5">
        <Card className="md:col-span-4 flex flex-col justify-between items-center text-center border-none bg-gradient-to-tr from-blue-600 via-indigo-600 to-cyan-500 shadow-[0_20px_50px_rgba(59,130,246,0.15)] rounded-[32px] p-8 relative overflow-hidden text-white min-h-[260px]">
          ...
          <div className="space-y-1 relative z-10">
            <span className="text-[10px] font-black uppercase tracking-[0.25em] text-cyan-200">💰 Estimasi Total Cost</span>
            <h3 className="text-xs text-blue-100 font-bold leading-relaxed">Hasil Perhitungan Biaya ML</h3>
          </div>
          <div className="my-4 relative z-10">
            <span className="text-5xl font-black text-white tracking-tight drop-shadow-sm">{predictionResult.formatted || formatUSD(safeBiaya)}</span>
          </div>
        ...
        <Card className="md:col-span-4 p-8 border border-slate-200/50 flex flex-col justify-between items-center text-center rounded-[32px] bg-white/80 backdrop-blur-xl shadow-[0_8px_30px_rgb(0,0,0,0.02)] min-h-[260px]">
          <div className="pb-3 border-b border-slate-100 w-full">
            <h3 className="text-xs font-black text-slate-500 uppercase tracking-widest flex items-center justify-center gap-1.5">⚙️ Status Efisiensi CPU</h3>
          </div>
          <div className="my-4 flex flex-col items-center gap-3 w-full">
            <span className="text-4xl font-black text-slate-800">{(cpuEfficiency / 100).toFixed(2)}</span>
            ...
            <p className="text-[10px] text-slate-400 font-bold max-w-[180px] leading-relaxed">Rasio jam CPU aktual vs required (Utilisasi: <strong className="text-slate-700">{input.CPU_Utilization}%</strong>).</p>
          </div>
          <div className="text-[9px] text-slate-400 font-bold uppercase tracking-wider">Target Efisiensi: 1.00</div>
        </Card>

        <Card className="md:col-span-4 p-8 border border-slate-200/50 flex flex-col justify-between items-center text-center rounded-[32px] bg-white/80 backdrop-blur-xl shadow-[0_8px_30px_rgb(0,0,0,0.02)] min-h-[260px]">
          <div className="pb-3 border-b border-slate-100 w-full">
            <h3 className="text-xs font-black text-slate-500 uppercase tracking-widest flex items-center justify-center gap-1.5">📌 Rekomendasi Sistem</h3>
          </div>
          <div className="my-4 flex flex-col items-center gap-3">
            <span className={`text-lg font-black uppercase px-5 py-2.5 rounded-2xl shadow-sm ${analisis.status_beban_kerja === "Kelebihan Beban" ? "bg-rose-100 text-rose-700" : analisis.status_beban_kerja === "Kurang Dimanfaatkan" ? "bg-amber-100 text-amber-700" : "bg-emerald-100 text-emerald-700"}`}>
            ...
            <p className="text-[9.5px] text-slate-500 font-bold max-w-[200px] leading-relaxed">{analisis.rekomendasi_tindakan}</p>
          </div>
          <div className="text-[9px] text-slate-400 font-bold uppercase tracking-wider">FinOps AI Recommendation</div>
        </Card>
      </div>
      ...
      <Card className="border border-slate-200/60 rounded-[28px] bg-white/85 backdrop-blur-xl shadow-[0_8px_30px_rgb(0,0,0,0.02)] overflow-hidden">
        <div className="px-7 pt-6 pb-4 border-b border-slate-100/80">
          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 rounded-xl bg-gradient-to-tr from-indigo-500 to-blue-500 flex items-center justify-center shadow-md"><Layers size={15} className="text-white" /></div>
            <div>
              <h3 className="text-sm font-black text-slate-800 tracking-tight">Parameter Prediksi yang Digunakan</h3>
              <p className="text-[10px] font-bold text-slate-400 mt-0.5">Semua parameter input yang dikirim ke model ML</p>
            </div>
          </div>
        </div>
        <div className="px-7 py-5">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-x-8">
            <div>
              <div className="text-[9px] font-black text-blue-500 uppercase tracking-[0.2em] mb-2 flex items-center gap-1.5"><Server size={10} /> Konfigurasi</div>
            ...
            <div>
              <div className="text-[9px] font-black text-emerald-500 uppercase tracking-[0.2em] mb-2 flex items-center gap-1.5"><Cpu size={10} /> Kapasitas CPU</div>
              <div className="divide-y divide-slate-100">
            ...
            <div>
              <div className="text-[9px] font-black text-amber-500 uppercase tracking-[0.2em] mb-2 flex items-center gap-1.5"><CreditCard size={10} /> Penyimpanan</div>
              <div className="divide-y divide-slate-100">
